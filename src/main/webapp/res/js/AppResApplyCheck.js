...
function initListener() {
	...
	$("#btn_check_pass").bind("click", checkPass);
	$("#btn_check_back").bind("click", checkBack);
}
function initFace() {
}
...
function query(pageNum){
	...
	RS.ajax({url:"/res/appres/queryApplyInfoPage",ps:cdt,cb:function(r) {
		if(!CU.isEmpty(r)){
			var data = r.data;
...
function showCheckForm(info) {
	...
	$("#div_checkFormTitle").html("应用[<font color='blue'>"+info.app.appCode+"</font>][<font color='blue'>"+info.app.appName+"</font>]资源申请单[<font color='#008800'>"+info.apply.code+"</font>]审核");
	$('#div_checkForm').modal('show');
}
function showCheckView(info) {
	$("#tabCheckView").html("");
	$("#div_checkViewTitle").html("应用[<font color='blue'>"+info.app.appCode+"</font>][<font color='blue'>"+info.app.appName+"</font>]资源申请单[<font color='#008800'>"+info.apply.code+"</font>]审核情况");
...
function checkApply(ct) {
	...
	CC.showMsg({msg:"您确定要审核[<font color='"+(ct==1?"#008800":"red")+"'>"+(ct==1?"通过":"退回")+"</font>]当前应用资源申请单吗?",option:2,callback:function(cr) {
		if(cr != "ok") return ;
		var url = "/res/appres/"+(ct==1?"/checkApplyPass":"/checkApplyBack");
		RS.ajax({url:url,ps:{applyId:CurrentCheckApply.apply.id,checkDesc:desc,cb:function() {
